...
describe('router-post.test.js', () => {
  ...
  describe('GET /api/posts', () => {
    let fetchPage = (page) => {
      return request(`${API_URL}/posts?page=${page}`)
      .catch(err => err)
    }
    ...
    test('should respond with 100 posts', () => {
      return mockManyPosts()
      .then(postsData => {
        return fetchPage(1)
        .then(res => {
          expect(res.status).toEqual(200)
    ...
    test('should respond with 50 posts', () => {
      ...
      .then(postsData => {
        ...
        .then(res => {
          ...
          expect(res.body.prev).toEqual(`${API_URL}/posts?page=1`)
          expect(res.body.next).toEqual(null)
          expect(res.body.last).toEqual(`${API_URL}/posts?page=2`)
  ...
  describe('GET /api/posts/:id', () => {
    test('should fetch a post', () => {
      return mockPost()
      .then(mock => {
        return request.get(`${API_URL}/posts/${mock.post._id}`)
        .then(res => {
          expect(res.status).toEqual(200)
        })
      })
    ...
    test('should 404', () => {
      return mockPost()
      .then(mock => {
        return request.get(`${API_URL}/posts/${mock.post.owner}`)
        .catch(res => res)
        .then(res => {
  ...
  describe('PUT /api/posts/:id', () => {
    test('should respond with updated post', () => {
      return mockPost()
      .then(mock => {
        return request.put(`${API_URL}/posts/${mock.post._id}`)
  ...
  describe('DELETE /api/posts/:id', () => {
    test('should delete a post', () => {
      return mockPost()
      .then(mock => {
        return request.delete(`${API_URL}/posts/${mock.post._id}`)
        .set('Authorization', `Bearer ${mock.userData.token}`)
        .then(res => {
    ...
    test('should 404', () => {
      return mockPost()
      .then(mock => {
        return request.delete(`${API_URL}/posts/${mock.post.owner}`)
        .set('Authorization', `Bearer ${mock.userData.token}`)
        .catch(res => res)
        .then(res => {
          expect(res.status).toEqual(404)
